...
1. **Раскладка страницы вакансий.** Сейчас фильтры лежат над списком. На whitesnake.by/vacancies — левый рельс с фильтрами и список справа; для работы со списком это объективно лучше: фильтры видны при прокрутке.
2. **Хиро съедает целый экран** ради одной фразы. На странице, чья задача — показать вакансии, первый экран должен подводить к списку, а не заменять его.
3. **Ничего не работает:** фильтры не фильтруют, поиск не ищет, форма не отправляется. Нужен хотя бы клиентский фильтр и заглушка отправки.
4. **Вакансии зашиты в компонент** (`VacancyList.tsx`) — вынести в данные.
...
7. **Ширины не согласованы:** контент 1040, подвал 1500 — подвал выглядит шире всей страницы.
8. **Макет и код разошлись:** в Figma `HeroScreen` всё ещё с росчерками и медиа-карточкой, в коде — простой центрированный текст.

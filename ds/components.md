...
| Имя | Тип | Node ID | Матрица | Used in | Назначение |
|-----|-----|---------|---------|---------|------------|
| `Input` | ComponentSet | `86:27` | `State: Default\|Hover\|Focus\|Error\|Disabled` | — | Текстовое поле. `radius/16`, высота ≈56 |
| `FileUpload` | ComponentSet | `86:46` | `State: Default\|Hover\|Filled` | — | Загрузка резюме. Пунктирная граница, `IconUpload` |
...
| `Select` | ComponentSet | `127:81` | `State: Default\|Hover\|Open\|Selected\|Disabled` | SearchFilters (HR Site) | Селект **в стиле поля `Input`**: высота `spacing/56`, `radius/16`, паддинги 20, бордер `input/*`, текст `body-m`, шеврон справа (в Open развёрнут), в Selected — выбранное значение тёмным. Токены `select/*` (10) ведут на ту же семантику, что и `input/*` |
...
| `Avatar` | ComponentSet | `89:27` | `Variant: Initials\|IconFallback` | — | Круглый аватар |
